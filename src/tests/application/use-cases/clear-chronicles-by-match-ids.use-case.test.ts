...
describe("ClearChroniclesByMatchIdsUseCase", () => {
  let useCase: ClearChroniclesByMatchIdsUseCase;
  let chronicleRepository: ChronicleRepository;
  let matchRepository: MatchRepository;

  beforeEach(async () => {
    useCase = await DIContainer.getClearChroniclesByMatchIdsUseCase();
    chronicleRepository = DIContainer.getChronicleRepository();
    matchRepository = DIContainer.getMatchRepository();
  });

  it("should delete only chronicles that belong to the requested match ids", async () => {
  ...
  it("should not affect matches when deleting chronicles by match ids", async () => {
    const league = await buildLeague();
    const season = await buildSeason(league._id);
    const home = await buildTeam();
    const away = await buildTeam();
